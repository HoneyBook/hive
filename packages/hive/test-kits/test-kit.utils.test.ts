import { buildTestKitRecordFromArray, collectImplicitDependenciesDeep } from "./test-kit.utils";
...
class MockUserTestKit extends TestKit {
  ...
  > {
    return [MockApiAdapterTestKit, MockConfigTestKit, MockAccountTestKit] as DependsOn<
      [MockApiAdapterTestKit, MockConfigTestKit, MockAccountTestKit]
    >;
  }
}
...
class MockCompanyTestKit extends TestKit {
  ...
  get dependentTestKitClasses(): DependsOn<[MockApiAdapterTestKit, MockUserTestKit]> {
...
class MockProjectTestKit extends TestKit {
  ...
  > {
    return [MockApiAdapterTestKit, MockUserTestKit, MockCompanyTestKit] as DependsOn<
      [MockApiAdapterTestKit, MockUserTestKit, MockCompanyTestKit]
    >;
  }
}
...
describe("test-kit.utils", () => {
  ...
  describe("collectImplicitDependenciesDeep", () => {
    ...
    it("should collect single level dependencies", () => {
      ...
      expect(result.MockApiAdapterTestKit).toBeInstanceOf(MockApiAdapterTestKit);
    ...
    it("should collect multi-level dependencies without duplicates", () => {
      ...
      expect(result.MockAccountTestKit).toBeInstanceOf(MockAccountTestKit);
    });
    ...
    it("should collect complex dependency tree without duplicates", () => {
      ...
      expect(result.MockCompanyTestKit).toBeInstanceOf(MockCompanyTestKit);
    });
    ...
    it("should reuse provided TestKit instances", () => {
      ...
      const dependencyNames = Object.keys(result).sort();
      expect(dependencyNames).toEqual(["MockAccountTestKit", "MockConfigTestKit"]);
    ...
    it("should handle circular dependencies without infinite loop", () => {
      ...
      expect(result.MockCircularBTestKit).toBeInstanceOf(MockCircularBTestKit);
    });
    ...
    it("should maintain instance uniqueness across complex scenarios", () => {
      ...
      expect(
        Object.values(result).filter(
          (instance) => instance.constructor.name === "MockApiAdapterTestKit",
        ).length,
      ).toBe(1);
    });
    ...
    it("should preserve dependency order and relationships", () => {
      ...
      const userDependencies = userTestKitInstance.dependentTestKitClasses;
  ...
  describe("integration scenarios", () => {
    it("should work end-to-end with realistic TestAppRunner scenario", () => {
      ...
      const impliedDependencies = collectImplicitDependenciesDeep(testKitsMap);
      ...
      expect(allTestKits.MockApiAdapterTestKit).toBeInstanceOf(MockApiAdapterTestKit); // Implied
      expect(allTestKits.MockConfigTestKit).toBeInstanceOf(MockConfigTestKit); // Implied
    });
  });
});
